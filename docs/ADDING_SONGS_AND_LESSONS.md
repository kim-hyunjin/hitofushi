...
- 파일명이 문법 ID가 됩니다. 예를 들어 `teiru.md`는 단원 JSON의
  `"grammarIds": ["teiru"]`와 연결되며 클릭할 때 팝오버로 열립니다.
...
- `src/pages/songs/index.astro`: 노래 목록의 곡 카드와 전체 통계
- `src/pages/my-learning.astro`: 모든 단원을 자동 수집하므로 별도 수정 불필요

새 곡을 대표 콘텐츠로 보여 주려면 홈 화면을 갱신하고, 사용자가 곡을 찾을 수
있도록 노래 목록에 새 카드를 추가합니다.
...
- 곡 소개 → 단원 링크와 문법/한자 팝오버가 모두 열리는가
